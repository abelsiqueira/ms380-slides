...
\myframe {
  Dado uma sequência real $(a_n)$, se existir $L \in \mathbb{R}$ tal que
  para todo $\varepsilon > 0$, existe $n_0 \in \mathbb{N}$ tal que
  $\vert{a_n - L}\vert < \varepsilon$ para todo $n > n_0$.
...
\myframe {
  Dado uma função $f$, com $a \in \mathbb{R}$, se existe $L \in \mathbb{R}$
  tal que, para todo $\varepsilon > 0$, existe $\delta > 0$ (que depende
  de $\varepsilon$) tal que
  $$\mbox{se } \qquad 0 < \vert{x - a}\vert < \delta, \qquad \mbox{então} \qquad
